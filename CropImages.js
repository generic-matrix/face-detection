...
async function CropImages(uri,json) {
    return new Promise(async (resolve, reject) => {
        try{
            let result = []
            var index = 0
            await json.responses[0].faceAnnotations.forEach(async (faceAnnotation)=>{
                boundingPoly = faceAnnotation.boundingPoly
                ...
                result.push(res.uri);
                if((json.responses[0].faceAnnotations.length-1)===index){
                    resolve(result)
                }
                index = index + 1
            })
